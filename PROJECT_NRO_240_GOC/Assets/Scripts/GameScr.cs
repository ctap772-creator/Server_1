...
public class GameScr : mScreen, IChatable
{
	...
	public void createMenu(string[] menu, Npc npc)
	{
		...
		if (ModFunc.GI().ShouldAddNhapNgocRongMenuCommand(menu, npc))
		{
			myVector.addElement(ModFunc.GI().CreateNhapNgocRongMenuCommand(npc));
		}
	...
	public void readPart()
	{
		...
		try
		{
			sbyte[] array = Rms.loadRMS("NR_part");
			if (array == null)
			{
				if (parts == null)
				{
			...
			parts = new Part[System.Math.Max(partSize, 5000)];
			for (int i = 0; i < parts.Length; i++)
			{
				if (i < partSize)
				{
	...
	public void doiMauTroi()
	{
		...
		int num = GameCanvas.skyColor;
		if (GameCanvas.colorTop != null && GameCanvas.colorTop.Length > 0)
		{
			num = GameCanvas.colorTop[GameCanvas.colorTop.Length - 1];
		}
		mautroi = mGraphics.blendColor(0.4f, 0, num);
	}
	...
	public bool isRongThanInCurrentMap()
	{
		return isRongThanXuatHien && TileMap.mapID == mapRID && TileMap.zoneID == zoneRID;
	}
	...
	public override void paint(mGraphics g)
	{
		...
		paint_ios_bg(g);
		if (isRongThanInCurrentMap() && TileMap.bgID != 3)
		{
			paintBlackSky(g);
		}
	...
	private void paintInfoBar(mGraphics g)
	{
		...
		else
		{
			...
			{
				Npc npcFocus = Char.myCharz().npcFocus;
				string npcFocusName = npcFocus.template.name;
				if (ModFunc.isShowID)
				{
					npcFocusName = "[" + npcFocus.template.npcTemplateId + "] " + npcFocusName;
				}
				mFont.tahoma_7b_green2.drawString(g, npcFocusName, imgScrW / 2, 9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
	...
	public static void paintHPBar_NEW(mGraphics g, int x, int y, Char c)
	{
		...
		{
			string npcFocusName2 = c.npcFocus.template.name;
			if (ModFunc.isShowID)
			{
				npcFocusName2 = "[" + c.npcFocus.template.npcTemplateId + "] " + npcFocusName2;
			}
			mFont.tahoma_7_green2.drawString(g, npcFocusName2, x3, y2, 2);
		}
