...
public class SoundMn
{
	...
	public void GetStrModFunc()
	{
		...
		(bool, string)[][] modFuncByTab = new(bool, string)[4][]
		{
			new(bool, string)[5]
			{
				(ModFunc.GI().fpsMode > 0, ModFunc.GI().GetFpsModeLabel()),
			...
			new(bool, string)[4]
			{
				(ModFunc.GI().isAutoPhaLe, ModFunc.GI().GetAutoPhaLeLabel()),
				(ModFunc.GI().autoWakeUp, ModFunc.strAutoWakeUp),
				(ModFunc.isAutoLogin, ModFunc.strAutoLogin),
				(ModFunc.GI().isAutoNhapNgocRong, ModFunc.strAutoNhapNgocRong)
			},
			new(bool, string)[6]
			{
			...
			new(bool, string)[6]
			{
				...
				(ModFunc.deVeNhaKhiTachHT, ModFunc.strDeVeNhaKhiTachHT),
				(false, "Auto Pha Lê Hóa/Lần: " + ModFunc.GI().GetAutoPhaLeBatchName())
			}
		};
