...
public class mSystem
{
    ...
    public static FrameImage getFraImage(string nameImg)
    {
        ...
        MainImage mainImage = ImgByName.getImagePath(nameImg, ImgByName.hashImagePath);
        if (mainImage != null && mainImage.img != null)
        {
            int nFrame = mainImage.nFrame;
            if (nFrame < 1)
            {
                nFrame = 1;
            }
            int imgH = mainImage.img.getHeight();
            int imgW = mainImage.img.getWidth();
            if (imgH < 1 || imgW < 1)
            {
                return null;
            }
            ...
            result = new FrameImage(mainImage.img, imgW, num);
        }
        return result;
    }

    public static void drawFraImage(FrameImage fraImage, int tickDiv, int x, int y, int trans, int anchor, mGraphics g)
    {
        if (fraImage == null || fraImage.nFrame < 1)
        {
            return;
        }
        int frame = (tickDiv < 1) ? 0 : (GameCanvas.gameTick / tickDiv % fraImage.nFrame);
        fraImage.drawFrame(frame, x, y, trans, anchor, g);
    }
