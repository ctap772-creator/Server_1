...
{
	private const string EncryptedDefaultServerList = "DmJ7(){;LzL$W#e8U=tPmbF?b0U6@2k^bMhna?TLVMaey$=jrF$&h0;apF~`nq~jD%9ThrdZjh1i$?u9*%QDXSkq|tBzw9g`)s>V{Zb&!!n9xBa<rvT=F3k>";

	private const int DefaultServerListKey = 113;

	public static readonly string DefaultServerList = ModFunc.Decrypt(EncryptedDefaultServerList, DefaultServerListKey);
	...
	public static string ListIP = DefaultServerList;
	...
	public static string linkDefault = DefaultServerList;
	...
	public static void GetServerList(string str, bool resetConnectionState = true)
	{
		...
		if (array.Length < 3)
		{
			array = Res.split(DefaultServerList, ",", 0);
		}
		...
		if (resetConnectionState || hasConnected == null || hasConnected.Length != nameServer.Length)
		{
			hasConnected = new bool[nameServer.Length];
		}
