...
public class Rms
{
	...
	public static void saveRMSDirect(string filename, sbyte[] data)
	{
		try
		{
			string text = GetiPhoneDocumentsPath() + "/" + filename;
		...
		{
			Debug.LogError("Error saving RMS direct: " + ex.Message);
		}
	}

	public static sbyte[] loadRMSDirect(string filename)
	{
		try
		{
			byte[] array;
