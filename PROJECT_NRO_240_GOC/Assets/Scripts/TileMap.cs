...
public class TileMap
{
	...
	public static void getTile()
	{
		...
		if (image != null)
		{
			...
			{
				imgTile[l] = GameCanvas.loadImageRMSTilePart("/t/" + tileID + "$" + (l + 1) + ".png", zoom);
			}
		}
		else
		{
	...
	public static void paintTile(mGraphics g, int frame, int indexX, int indexY)
	{
		if (imgTile != null)
		{
			...
			else if (frame >= 0 && frame < imgTile.Length && imgTile[frame] != null)
			{
	...
	public static bool hasAnyTileLoaded()
	{
		if (imgTile == null)
		{
			return false;
	...
	public static void loadMainTile()
	{
		...
		{
			getTile();
			if (!hasAnyTileLoaded() && Main.isIPhone)
			{
				GameScr.info1.addInfo("Thiếu tile map (tile " + tileID + "). Xóa data app và tải lại game.", 0);
			}
			lastTileID = tileID;
		}
	}
