...
{
	private const string AutoBuyCaption = "Mua nhi\u1ec1u";

	private const string AutoBuyQuantityPrompt = "Nh\u1eadp S\u1ed1 L\u01b0\u1ee3ng Item C\u1ea7n Mua";

	private const string AutoBuyQuantityHint = "Ch\u1ec9 \u0110\u01b0\u1ee3c Nh\u1eadp S\u1ed1";
	...
	private bool isAutoCombineFast;
	...
	private bool isBossTop()
	{
		return !isThachDau && topName != null && topName.Equals("Boss");
	}
	...
	{
		int combineButtonCount = (vItemCombine.size() > 0 && ModFunc.ShouldShowAutoDapDoButton(this)) ? 2 : 1;
		currentListLength = vItemCombine.size() + combineButtonCount;
		ITEM_HEIGHT = 29;
		cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
		if (cmyLim < 0)
		{
	...
	private void updateKeyCombine()
	{
		if (currentTabIndex == 0)
		{
			...
			if (ModFunc.ShouldShowAutoDapDoButton(this) && selected == vItemCombine.size() + 1 && GameCanvas.isPointerClick)
			{
				GameCanvas.isPointerClick = false;
				keyTouchCombine = 2;
			}
	...
	{
		ITEM_HEIGHT = 30;
		currentListLength = ensurePetSkills(isPet2).Length + 5;
		cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
		if (cmyLim < 0)
		{
			cmyLim = 0;
	...
	{
		ITEM_HEIGHT = 30;
		Item[] arrItemBody = ensurePetBodyItems(isPet2);
	...
	private Item[] ensurePetBodyItems(bool isPet2)
	{
		Char @char = (isPet2 ? Char.MyPet2z() : Char.myPetz());
		if (@char.arrItemBody == null)
		{
			@char.arrItemBody = new Item[0];
		}
		return @char.arrItemBody;
	}

	private Skill[] ensurePetSkills(bool isPet2)
	{
		Char @char = (isPet2 ? Char.MyPet2z() : Char.myPetz());
		if (@char.arrPetSkill == null)
		{
			@char.arrPetSkill = new Skill[0];
		}
		return @char.arrPetSkill;
	}
	...
	private void paintPetSkill(mGraphics g, bool isPet2)
	{
		...
		Skill[] arrPetSkill = ensurePetSkills(isPet2);
		int num = 5 + arrPetSkill.Length;
		for (int i = 0; i < num; i++)
		{
			int num2 = xScroll + 30;
			int num3 = yScroll + i * ITEM_HEIGHT;
			...
			if (i == 4)
			{
				...
				int num11 = 0;
				string st5 = mResources.critical + " " + mResources.root + ": 0%";
				if (num11 > t_tiemnang.Length - 1)
				{
					num11 = t_tiemnang.Length - 1;
				}
				_ = t_tiemnang[num11];
	...
	private void paintCombine(mGraphics g)
	{
		...
		int combineButtonCount = (vItemCombine.size() > 0 && ModFunc.ShouldShowAutoDapDoButton(this)) ? 2 : 1;
		for (int j = 0; j < vItemCombine.size() + combineButtonCount; j++)
		{
			int num = xScroll + 29;
			...
			if (j >= vItemCombine.size())
			{
				if (vItemCombine.size() > 0)
				{
					int buttonIndex = j - vItemCombine.size();
					string buttonLabel = (buttonIndex == 0) ? mResources.UPGRADE : "Auto";
					int buttonTouchKey = buttonIndex + 1;
					if (!GameCanvas.isTouch && j == selected)
					{
						g.setColor(16383818);
						g.fillRect(num5, num2, wScroll, num4 + 2);
					}
					if ((j == selected && keyTouchCombine == buttonTouchKey) || (!GameCanvas.isTouch && j == selected))
					{
						g.drawImage(GameScr.imgLbtnFocus, xScroll + wScroll / 2, num2 + num4 / 2 + 1, StaticObj.VCENTER_HCENTER);
						mFont.tahoma_7b_green2.drawString(g, buttonLabel, xScroll + wScroll / 2, num2 + num4 / 2 - 4, mFont.CENTER);
					}
					else
					{
						g.drawImage(GameScr.imgLbtn, xScroll + wScroll / 2, num2 + num4 / 2 + 1, StaticObj.VCENTER_HCENTER);
						mFont.tahoma_7b_dark.drawString(g, buttonLabel, xScroll + wScroll / 2, num2 + num4 / 2 - 4, mFont.CENTER);
					}
				}
				continue;
			}
	...
	private void paintPetInfo(mGraphics g, bool isPet2)
	{
		...
		if (pet.cPower > 0.0)
		{
			string text = (!pet.me) ? pet.currStrLevel : pet.getStrLevel();
			mFont.tahoma_7_yellow.drawString(g, fitHeaderText(mFont.tahoma_7_yellow, text, W - 70), X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
		}
		if (pet.cDamFull > 0.0)
		{
	...
	private void paintPetSkillInfo(mGraphics g, bool isPet2)
	{
		...
		mFont.tahoma_7_yellow.drawString(g, mResources.critical + ": " + pet.cCriticalFull + "%", X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
		mFont.tahoma_7_yellow.drawString(g, mResources.armor + ": " + NinjaUtil.getMoneys(pet.cDefull), X + 120, 27, mFont.LEFT, mFont.tahoma_7_grey);
	...
	private string fitHeaderText(mFont font, string text, int maxWidth)
	{
		if (text == null)
		{
			return string.Empty;
		...
		string suffix = "...";
		while (text.Length > 0 && font.getWidth(text + suffix) > maxWidth)
		{
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
		...
		if (c.cPower > 0.0)
		{
			string text = (!c.me) ? c.currStrLevel : c.getStrLevel();
			mFont.tahoma_7_yellow.drawString(g, fitHeaderText(mFont.tahoma_7_yellow, text, W - 70), X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
		}
	...
	private void paintPetStatusInfo(mGraphics g, bool isPet2)
	{
		...
		mFont.tahoma_7b_white.drawString(g, "HP: " + NinjaUtil.getMoneys(pet.cHP) + "/" + NinjaUtil.getMoneys(pet.cHPFull), X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		mFont.tahoma_7b_white.drawString(g, "MP: " + NinjaUtil.getMoneys(pet.cMP) + "/" + NinjaUtil.getMoneys(pet.cMPFull), X + 60, 16, mFont.LEFT, mFont.tahoma_7b_dark);
		mFont.tahoma_7_yellow.drawString(g, mResources.critical + ": " + NinjaUtil.getMoneys(pet.cCriticalFull) + "%", X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
		mFont.tahoma_7_yellow.drawString(g, mResources.armor + ": " + NinjaUtil.getMoneys(pet.cDefull), X + 120, 27, mFont.LEFT, mFont.tahoma_7_grey);
	...
	private void DoFirePet2Main()
	{
		if (currentTabIndex == 0)
		{
			Item[] arrItemBody = ensurePetBodyItems(isPet2: true);
			if (selected != -1 && selected <= arrItemBody.Length - 1)
			{
				MyVector myVector = new MyVector(string.Empty);
				Item item = arrItemBody[selected];
				currItem = item;
				if (currItem != null)
				{
	...
	private void doFirePetMain()
	{
		if (currentTabIndex == 0)
		{
			Item[] arrItemBody = ensurePetBodyItems(isPet2: false);
			if (selected != -1 && selected <= arrItemBody.Length - 1)
			{
				MyVector myVector = new MyVector(string.Empty);
				Item item = arrItemBody[selected];
	...
	private void doFireTop()
	{
		if (selected >= -1)
		{
			...
			MyVector myVector = new MyVector(string.Empty);
			TopInfo topInfo = (TopInfo)vTop.elementAt(selected);
			myVector.addElement(new Command(mResources.CHAR_ORDER[0], this, 9999, topInfo));
			if (isBossTop())
			{
				myVector.addElement(new Command("H\u1ed3i sinh", this, 10006, topInfo));
			}
			GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
			addThachDauDetail(topInfo);
		}
	}
	...
	private void doFireCombine()
	{
		if (currentTabIndex == 0)
		{
			...
			if (ModFunc.ShouldShowAutoDapDoButton(this) && selected == vItemCombine.size() + 1)
			{
				keyTouchCombine = -1;
				selected = (GameCanvas.isTouch ? (-1) : 0);
				if (ModFunc.GI().isAutoDapDo)
				{
					ModFunc.GI().RequestStartAutoDapDo();
				}
				else
				{
					GameCanvas.startYesNoDlg("Bạn có muốn bật Auto nâng cấp đồ không ?", new Command(mResources.YES, this, 6002, null), new Command(mResources.NO, this, 10020, null));
				}
				return;
			}
	...
	private void showAutoBuyQuantityInput(Item item)
	{
		...
		currItem = item;
		chatTField.tfChat.setText(string.Empty);
		chatTField.strChat = AutoBuyQuantityPrompt;
		chatTField.tfChat.name = AutoBuyQuantityHint;
		chatTField.to = string.Empty;
		chatTField.isShow = true;
		chatTField.tfChat.isFocus = true;
		chatTField.tfChat.setIputType(TField.INPUT_TYPE_NUMERIC);
		chatTField.tfChat.setMaxTextLenght(6);
		if (chatTField.right != null)
		{
			chatTField.right.caption = mResources.DELETE;
		}
		if (GameCanvas.isTouch)
		{
			chatTField.tfChat.doChangeToTextBox();
	...
	private sbyte getAutoBuyType(Item item)
	{
		if (item.isBuySpec)
		{
			return 3;
	...
	private void buyManyFromInput(string text)
	{
		int quantity;
		if (currItem == null || currItem.template == null || !int.TryParse(text, out quantity) || quantity <= 0)
		{
			GameCanvas.startOKDlg(mResources.input_quantity_wrong);
			return;
		}
		sbyte buyType = getAutoBuyType(currItem);
		chatTField.isShow = false;
		chatTField.tfChat.setIputType(TField.INPUT_TYPE_ANY);
		chatTField.tfChat.setMaxTextLenght(80);
		ModFunc.GI().AutoBuyItem(quantity, currItem, buyType);
	}
			...
			if (currItem != null)
			{
				if (currItem.isBuySpec)
				{
					if (currItem.buySpec > 0)
					{
						...
						myVector.addElement(new Command(AutoBuyCaption, this, 3006, currItem));
					}
				}
				else if (typeShop == 4)
				{
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 9999)
		{
			...
			if (isBossTop())
			{
				Service.gI().sendAdminBossAction(topInfo.pId, 0);
			}
			else
			{
		...
		{
			TopInfo topInfo = (TopInfo)p;
			Service.gI().sendAdminBossAction(topInfo.pId, 1);
		}
		if (idAction == 170391)
		{
			Rms.clearAll();
		...
		{
			Item item14 = (Item)p;
			showAutoBuyQuantityInput(item14);
		}
		if (idAction == 4000)
		{
			Clan clan = (Clan)p;
	...
	public void onChatFromMe(string text, string to)
	{
		...
		else if (chatTField.strChat == "Nhập số sao cần đập" && chatTField.tfChat.getText() != string.Empty)
		{
			if (int.TryParse(text, out var maxPhale) && maxPhale > 0)
			{
				...
				if (!ModFunc.GI().isAutoPhaLe)
				{
					ModFunc.GI().isAutoPhaLe = true;
					new System.Threading.Thread(ModFunc.GI().AutoPhaLe).Start();
				}
			}
			else
			{
		...
		if (chatTField.strChat == AutoBuyQuantityPrompt)
		{
			buyManyFromInput(text);
			return;
		}
		if (chatTField.strChat == ModFunc.strThoiVangBanMoiLanPrompt)
		{
			if (int.TryParse(text, out var sl) && sl > 0)
			{
				ModFunc.slThoiVangCanBan = sl;
				ModFunc.SaveSlThoiVangCanBan();
				GameScr.info1.addInfo("Đã lưu " + ModFunc.strThoiVangBanMoiLan + ": " + sl, 0);
				if (type == 26)
				{
					SoundMn.gI().GetStrModFunc();
					ModFunc.GI().UpdateModFuncText(currentTabIndex);
					refreshModFuncListScroll();
				}
			}
			else
			{
	...
	public void setCombineEff(int type)
	{
		...
		if (ModFunc.GI().isAutoDapDo)
		{
			...
			speed = 40;
			isSpeedCombine = true;
			isAutoCombineFast = true;
		}
		else
		{
			isAutoCombineFast = false;
		}
	}

	private void updateCombineEff()
	{
		bool autoDapDoFast = isAutoCombineFast;
		if (autoDapDoFast)
		{
			countUpdate = 0;
		}
		...
		if (!isCompleteEffCombine)
		{
			if (time > 0)
			{
				if (combineSuccess != -1)
				{
					if (autoDapDoFast && typeCombine != 3)
					{
					...
					{
						if (GameCanvas.gameTick % (autoDapDoFast ? 2 : 10) == 0)
						{
			...
			else if (GameCanvas.gameTick % (autoDapDoFast ? 2 : 20) == 0)
			{
		...
		else
		{
			...
			if (combineSuccess == 1)
			{
				if (countWait == 10 || (autoDapDoFast && countWait == 4))
				{
				...
				if (rS < 300)
				{
					rS = Res.abs(rS + (autoDapDoFast ? 30 : 10));
					if (rS == 20)
					{
			...
			else
			{
				...
				if (countWait < (autoDapDoFast ? -12 : -50))
				{
					countWait = autoDapDoFast ? -12 : -50;
	...
	private void DoFireModFunc()
	{
		...
		case 0:
			...
			{
			case 0:
				ModFunc.GI().CycleFpsMode();
				GameScr.info1.addInfo("FPS: " + (ModFunc.GI().fpsMode == 0 ? "30" : (ModFunc.GI().fpsMode == 1 ? "60" : "120")), 0);
				break;
			case 1:
		...
		case 1:
			...
			{
			case 0:
				ModFunc.GI().ToggleAutoPhaLe();
				if (!ModFunc.GI().isAutoPhaLe)
				{
					GameScr.info1.addInfo("Đã tắt tự động pha lê hóa", 0);
				}
				else
				{
					GameScr.info1.addInfo("Đã bật tự động pha lê hóa: " + ModFunc.GI().GetAutoPhaLeBatchName(), 0);
				}
				break;
			case 1:
			...
			case 2:
				...
				break;
			case 3:
				ModFunc.GI().ToggleAutoNhapNgocRong();
				GameScr.info1.addInfo("\u0110\u00E3 " + (ModFunc.GI().isAutoNhapNgocRong ? "b\u1EADt" : "t\u1EAFt") + " t\u1EF1 \u0111\u1ED9ng nh\u1EADp ng\u1ECDc r\u1ED3ng", 0);
				break;
			}
			break;
		case 2:
		...
		case 3:
			...
			case 3:
				...
				break;
			case 4:
				ModFunc.changeStatusDeVeNhaKhiTachHT();
				GameScr.info1.addInfo("\u0110\u00E3 " + (ModFunc.deVeNhaKhiTachHT ? "b\u1EADt" : "t\u1EAFt") + " cho \u0111\u1EC7 v\u1EC1 nh\u00E0 khi t\u00E1ch HT", 0);
				break;
			case 5:
				ModFunc.GI().CycleAutoPhaLeBatchModeConfig();
				GameScr.info1.addInfo("Số lần đập: " + ModFunc.GI().GetAutoPhaLeBatchName(), 0);
				break;
			case 6:
				openSlThoiVangInput();
				return;
		...
		SoundMn.gI().GetStrModFunc();
		ModFunc.GI().UpdateModFuncText(currentTab);
		refreshModFuncListScroll();
	}

	private void openSlThoiVangInput()
	{
		if (chatTField == null)
		{
			chatTField = new ChatTextField();
		...
		ModFunc.GI().MyChatTextField(chatTField, ModFunc.strThoiVangBanMoiLanPrompt, "Chỉ nhập số");
		chatTField.tfChat.setText(ModFunc.slThoiVangCanBan.ToString());
	}

	private void refreshModFuncListScroll()
	{
		if (type != 26)
		{
			return;
