...
public class GameCanvas : IActionListener
{
	...
	public void onConnectionFail()
	{
		if (currentScreen.Equals(SplashScr.instance))
		{
			...
			{
				ServerListScreen.GetServerList(ServerListScreen.linkDefault, false);
				int nextServer = -1;
				for (int i = 0; i < ServerListScreen.hasConnected.Length; i++)
				{
				...
				if (nextServer != -1)
				{
					ServerListScreen.hasConnected[nextServer] = true;
					ServerListScreen.ipSelect = nextServer;
					GameMidlet.IP = ServerListScreen.address[ServerListScreen.ipSelect];
					GameMidlet.PORT = ServerListScreen.port[ServerListScreen.ipSelect];
					Rms.saveRMSInt("svselect", ServerListScreen.ipSelect);
					connect();
	...
	public static void paintBackgroundtLayer(mGraphics g, int layer, int deltaY, int color1, int color2)
	{
		...
		{
			int num = layer - 1;
			if (num == imgBG.Length - 1 && GameScr.gI().isRongThanInCurrentMap())
			{
			...
			else
			{
				...
				{
					int fillColor = color2;
					if (fillColor == -1)
					{
						fillColor = (skyColor != 0) ? skyColor : 5197647;
					}
	...
	public static void paintBGGameScr(mGraphics g)
	{
		if (!isLoadBGok)
		{
			g.setColor((skyColor != 0) ? skyColor : 0);
			g.fillRect(0, 0, w, h);
		}
		...
		try
		{
			if (paintBG)
			{
				...
				else if (typeBg == 3)
				{
					...
					g.translate(0, -num);
					fillRect(g, (!GameScr.gI().isRongThanInCurrentMap()) ? colorTop[2] : GameScr.gI().mautroi, 0, num - (GameScr.cmy >> 3), gW, yb[2] - num + (GameScr.cmy >> 3) + 100, 2);
				...
				else if (typeBg == 10)
				{
					...
					g.translate(0, -num2);
					fillRect(g, (!GameScr.gI().isRongThanInCurrentMap()) ? colorTop[1] : GameScr.gI().mautroi, 0, num2 - (GameScr.cmy >> 2), gW, yb[1] - num2 + (GameScr.cmy >> 2) + 100, 2);
					paintBackgroundtLayer(g, 2, 2, -1, colorBotton[1]);
					drawSun1(g);
					drawSun2(g);
	...
	private static bool isBgLoaded()
	{
		if (imgBG == null)
		{
			return false;
	...
	private static string getRmsKey(string path, int zoom)
	{
		string text = Main.res + "/x" + zoom + path;
		text = cutPng(text);
		string[] array = Res.split(text, "/", 0);
		return "x" + zoom + array[array.Length - 1];
	}

	private static int getSafeBgH(int index)
	{
		if (imgBG != null && index >= 0 && index < imgBG.Length && imgBG[index] != null && bgH != null && index < bgH.Length && bgH[index] > 0)
		{
			return bgH[index];
		}
		return 100;
	...
	public static void loadBG(int typeBG)
	{
		try
		{
			...
			if (TileMap.lastBgID == typeBG && TileMap.lastType == TileMap.bgType && isBgLoaded())
			{
				return;
			}
			transY = 12;
		...
		{
			if (typeBg >= 0 && typeBg < StaticObj.SKYCOLOR.Length)
			{
				skyColor = StaticObj.SKYCOLOR[typeBg];
			}
			isLoadBGok = true;
	...
	public void paint(mGraphics gx)
	{
		try
		{
			...
			if (currentDialog != null && !ModFunc.GI().isAutoDapDo)
			{
				debug("PC", 1);
				currentDialog.paint(g);
			}
			else if (menu.showMenu && !ModFunc.GI().ShouldSuppressDapDoConfirmUi())
			{
				debug("PD", 1);
				menu.paintMenu(g);
			}
			...
			if (ChatPopup.currChatPopup != null && !ModFunc.GI().isAutoDapDo)
			{
				ChatPopup.currChatPopup.paint(g);
			}
			Hint.paint(g);
			if (ChatPopup.serverChatPopUp != null && !ModFunc.GI().ShouldSuppressDapDoConfirmUi())
			{
	...
	{
		Image image = loadImageRMS(path, mGraphics.zoomLevel);
		if (image != null)
		{
			return image;
		}
		int[] array = new int[4] { 2, 4, 3, 1 };
		for (int i = 0; i < array.Length; i++)
		{
			...
			image = loadImageRMS(path, num);
			if (image != null)
			{
				resizeImage(image, num);
				return image;
			}
		}
		return null;
	...
	public static Image loadImageRMSAtZoom(string path, int zoom)
	{
		Image image = loadImageRMSTilePart(path, zoom);
		if (image != null)
		{
			return image;
		}
		return loadImageRMS(path);
	}

	public static Image loadImageRMSTilePart(string path, int zoom)
	{
		try
		{
			sbyte[] data = Rms.loadRMS(getRmsKey(path, zoom));
			if (data != null)
			{
	...
	private static Image loadImageRMS(string path, int zoom)
	{
		if (Main.isIPhone)
		{
			return loadImageRMSTilePart(path, zoom);
		}
		string text = Main.res + "/x" + zoom + path;
		text = cutPng(text);
		try
		{
			return Image.createImage(text);
		}
		catch (Exception)
		{
			return loadImageRMSTilePart(path, zoom);
	...
	private static void resizeImage(Image img, int sourceZoom)
	{
		if (img == null || sourceZoom <= 0 || sourceZoom == mGraphics.zoomLevel)
		{
			return;
		}
		int num = img.w / sourceZoom;
		int hx1 = img.h / sourceZoom;
